...
<table><thead><tr><th>Requirement</th><th width="100">Type</th></tr></thead><tbody><tr><td>Secure API exposure: All APIs exposed via secure socket connections (HTTPS).</td><td>REQUIRED</td></tr><tr><td>Client application authorization tokens: Client applications must send authorization tokens in the authorization header of the request to authenticate users and the API Management Gateway will verify whether the token is valid.</td><td>REQUIRED</td></tr><tr><td>Transaction receipting: For each disbursement made by the system, a receipt should be issued to the recipient of the funds containing information about the transaction id, transaction date and time, reason for the payment, details of the payer, and the system should store all receipts issued for easy reference and reconciliation.</td><td>REQUIRED</td></tr><tr><td>Transaction status querying capability</td><td>REQUIRED</td></tr><tr><td>Display details of the transaction to the payer</td><td>REQUIRED</td></tr><tr><td>Transaction pre-approval: Prior to processing bulk payment and batch payment transactions, the transaction must be authorized and approved in the system.</td><td>REQUIRED</td></tr><tr><td>Pre-processing validation of data (well formed data and completeness checks) prior to disbursement.</td><td>REQUIRED</td></tr><tr><td>Ability to schedule bulk payments</td><td>REQUIRED</td></tr><tr><td>Support for batch payments</td><td>REQUIRED</td></tr><tr><td>Ability to lookup payment addresses</td><td>RECOMMENDED</td></tr><tr><td>Error handling and reconciliation</td><td>REQUIRED</td></tr><tr><td>Transaction status logging</td><td>REQUIRED</td></tr><tr><td>Triggering of payments</td><td>REQUIRED</td></tr></tbody></table>

### 5.3.2 Voucher Management

<table><thead><tr><th>Requirement</th><th width="100">Type</th></tr></thead><tbody><tr><td>Batch generation of vouchers</td><td>REQUIRED</td></tr><tr><td>Voucher uniqueness and randomness: Voucher pin must be unique and unpredictable.</td><td>REQUIRED</td></tr><tr><td>Secure voucher data storage</td><td>REQUIRED</td></tr><tr><td>High availability of storage</td><td>REQUIRED</td></tr><tr><td>Expose APIs that can be invoked by voucher serial number for purposes of querying a voucher, suspending, or unsuspending a voucher.</td><td>REQUIRED</td></tr><tr><td>Support an API that can be invoked to redeem a voucher using the voucher number.</td><td>REQUIRED</td></tr><tr><td>Support an API that can invoke payment gateway</td><td>REQUIRED</td></tr></tbody></table>

### 5.3.3 Mobile Payments

<table><thead><tr><th>Requirement</th><th width="100">Type</th></tr></thead><tbody><tr><td>Real time debiting/crediting of mobile money accounts</td><td>RECOMMENDED</td></tr><tr><td>Regular balance reconciliation with disbursement Agency</td><td>REQUIRED</td></tr><tr><td>Documented process for partners to dispute transaction records</td><td>REQUIRED</td></tr><tr><td>Target account identifier or lookup of payment address</td><td>REQUIRED</td></tr><tr><td>Ability to retrieve details of completed transactions in the batch</td><td>REQUIRED</td></tr></tbody></table>

### 5.3.4 Bulk Payments

<table><thead><tr><th>Requirement</th><th width="100">Type</th></tr></thead><tbody><tr><td>Ability to securely receive bulk payment requests as a single HTTPS request containing data for multiple transactions. The transaction data (with payment instructions data) is passed format and will be compatible with ISO 20022.</td><td>REQUIRED</td></tr><tr><td>The number of transactions that can be included in a single batch is limited by the size of the file upload and the processing time. If the number of transactions in the file exceeds the file size and could impact the performance of the system, the batch should be split into multiple batch requests.</td><td>REQUIRED</td></tr><tr><td>Batch files should be verified for any errors and validated as per business rules and regulations before it is accepted as a valid bulk payment file.</td><td>REQUIRED</td></tr><tr><td>Batch files containing duplicate payments will not be processed and an error will be generated.</td><td>REQUIRED</td></tr><tr><td>The bulk payment process has to be explicitly triggered by an authorized user. All requests to the Bulk Payment Application API must be authorized and digitally signed by the person initiating the bulk payment request.</td><td>REQUIRED</td></tr><tr><td>The batch file for bulk payments should contain the beneficiary ID token, and the amount to be paid. The payment information is not included in the batch file for security and privacy but is resolved by the verification and validation component of the bulk payment service by invoking the Account Lookup Directory Service (ALDS/ALS).</td><td>REQUIRED</td></tr><tr><td>The Bulk Payment Application API shall inspect the batch disbursement file and split transactions into bank payments and non-bank payments (e.g. Mobile money) in separate payment files before initiating the call to the Payment Gateway.</td><td>REQUIRED</td></tr><tr><td>The status of the bulk payment transaction can be obtained from the event log. The payment status code indicates the status of a single payment transaction and will be according to ISO 20022 Payment status codes table.</td><td>REQUIRED</td></tr></tbody></table>
